...
class VardictFilter():
    ...
    def filtering(self, genome, out_prefix=None, af_threshold=0.0, msi_af_threshold=0.0, dp_threshold=0, vd_threshold=0,
                  seq_error=None, black_error=None, tumor_index=1, filter_field:tuple=None,
                  center_size:tuple=(1, 1), basic_error=None):
        ...
        if seq_error:
            if seq_error.replace(".", "", 1).isdigit():
                ...
                # 从 json 文件提取条件概率对应的测序错误率
                seq_error_dict = json.load(open(seq_error))
        else:
            seq_error_dict = dict()

        if black_error is not None:
            ''' seq_error文件举例，xcue的提供的black list
            mutaion	AF_in_pop	mean_VAF
            chr10_10136339_C_G	0.8	0.7182
            chr10_103249036_A_G	0.933333	0.765007
            chr10_104263911_TG_T	0.0333333	0.0011
            '''
            black_error_dict = dict()
            with open(black_error) as f:
                _ = f.readline()
                for line in f:
                    lst = line.strip().split()
                    black_error_dict[lst[0]] = float(lst[2])
        else:
            black_error_dict = dict()

        if basic_error:
            # 从 json 文件提取总体测序错误率
            basic_error_dict = json.load(open(basic_error))
        else:
            basic_error_dict = dict()
        ...
        for r in self.vcf:
            ...
            if '<' in r.alts[0]:
                discard += 1
                # 跳过vardict中输出的特殊突变
                print('Skip SV mutation:', r.contig, r.ref, list(r.alts))
                continue

            # 硬性af过滤
            if r.samples[tumor]['AF'][0] < af_threshold:
                discard += 1
                continue

            # 硬性dp过滤
            dp = r.samples[tumor]['DP']
            if dp < dp_threshold:
                discard += 1
                continue

            # 硬性variant depth过滤
            vd = r.samples[tumor]['VD']
            if vd < vd_threshold:
                discard += 1
                continue

            # filter field?
            if filter_field and not (set(r.filter) & set(filter_field)):
                # print(set(r.filter), set(filter_field))
                discard += 1
                continue

            # 针对MSI区域的indel的AF过滤
            if ('MSI' in r.info and r.info['MSI'] >= 8) and (r.info['TYPE'] in ['Deletion', 'Insertion']) \
                    and (r.samples[tumor]['AF'][0] < msi_af_threshold):
                # print('MSI filtering')
                discard += 1
                continue
            ...
            if seq_error_dict or basic_error_dict:
                if len(r.alts[0]) == len(r.ref) == 1:
                    ...
                    if r.alts[0][0].upper() not in seq_error_dict:
                        print(f'warn: variant {r.contig}:{r.pos}:{r.ref}:{r.alts[0]} cannot be parsed!')
                    else:
            ...
            # 尝试从black error 提取error_rate
            if black_error_dict:
                key = '_'.join([r.contig, str(r.pos), r.ref, r.alts[0]])
                if key in black_error_dict:
                    black_error_rate = black_error_dict[key]
                    if black_error_rate > error_rate:
                        error_rate = black_error_rate
            if error_rate >= 1:
                error_rate = 0.999
            ...
            judge4 = self.pass_pstd(r, cutoff=0.0001)
            if not judge4[0]:
                reasons.append('pSTD')
...
def filterVcf(vcf, genome, out_prefix=None, af_threshold=0.0, vd_threshold=0,
              msi_af_threshold=0.0, dp_threshold=0, filter_field:tuple=None,
              seq_error=None, tumor_index:int=0, black_error=None,
              center_size:tuple=(1, 1), basic_error_dict=None):
    VardictFilter(vcf).filtering(
        out_prefix=out_prefix, seq_error=seq_error, black_error=black_error,
        tumor_index=tumor_index, genome=genome, af_threshold=af_threshold, vd_threshold=vd_threshold,
        msi_af_threshold=msi_af_threshold, dp_threshold=dp_threshold, filter_field=filter_field,
        center_size=center_size, basic_error=basic_error_dict
    )
